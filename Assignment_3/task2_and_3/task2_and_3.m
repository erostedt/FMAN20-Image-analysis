...
% Find an image with a face and one without:
face_pixels = X_test(:, find(Y_test == 1, 1));
not_face_pixels = X_test(:, find(Y_test == -1, 1));

% Predict respective image:
face_pred = classify(face_pixels, classification_data);
not_face_pred = classify(not_face_pixels, classification_data);

% Display images:
figure
imagesc(reshape(face_pixels, [19,19]))

figure
imagesc(reshape(not_face_pixels, [19,19]))

%Print predictions
fprintf('Prediction of face %d \n', face_pred)
fprintf('Prediction of not_face %d \n', not_face_pred)
